...
const checkContentTypeIsSet = (req, res, next) => {
  if (
    req.headers['content-length'] !== '0'
    ...
    res.status(400);
    res.set('Content-Type', 'application/json');
    res.json({ message: 'The "Content-Type" header must be set for requests with non-empty payloads' });
  }
  next();
};

const checkContentTypeIsJson = (req, res, next) => {
